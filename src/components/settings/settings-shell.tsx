  ...
  return (
    <section className={cn("px-2.5 py-2.5 md:px-3 md:py-3", className)}>
      <div className="flex items-center justify-between gap-3">
        ...
        <p className="max-w-[28rem] text-right font-mono text-[0.68rem] uppercase tracking-[0.06em] text-muted-foreground/60">
          {description}
        </p>
      </div>
  ...
  return (
    <section className="rounded-[3px] border border-border/50 bg-panel-strong/60 px-2.5 py-2">
      <div className="flex items-center justify-between gap-2">
        ...
        <p className="shrink-0 font-mono text-[0.79rem] font-semibold tabular-nums text-foreground">
          {value}
        </p>
      </div>
      {children}
...
export function MetricTile({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-[3px] border border-border/50 bg-panel-strong/60 px-2.5 py-1.5">
      <div className="flex items-center justify-between gap-2">
        <p className="font-mono text-[0.68rem] font-medium uppercase tracking-[0.08em] text-muted-foreground">
          {label}
        </p>
        <p className="font-mono text-[0.79rem] font-semibold tabular-nums tracking-[0.03em] text-foreground">
          {value}
        </p>
      </div>
    </div>
...
export function EndpointRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-[3px] border border-border/50 bg-panel-strong/60 px-2.5 py-1.5">
      ...
      <p className="mt-0.5 break-all font-mono text-[0.73rem] font-semibold tracking-[0.02em] text-foreground">
        {value}
      </p>
    </div>
  );
...
export function StateCallout({
  ...
  return (
    <div
      className={cn(
        "rounded-[3px] border px-2.5 py-1.5 font-mono text-[0.73rem] leading-4 tracking-[0.03em]",
        tone === "error"
